...
- **Code**: `lucas-maes/le-wm` + `stable-worldmodel` (env zoo / planner / dataset format) + `stable-pretraining` (training loop). HF checkpoints: `quentinll/lewm-{pusht,cube,tworooms,reacher}` ([howto](leworldmodel-howto.md)).
...
- **Compute footprint matches.** Orin Nano (~67 TOPS INT8) is plausibly enough for the LeWM ViT encoder + predictor at inference time. The full `pusht/lewm` checkpoint is 18M params per the [howto](leworldmodel-howto.md). Training stays off-board on a desktop GPU.
...
- **Sim-to-real gap.** Gazebo physics is functional but not high-fidelity for contact-rich manipulation. The sim-trained LeWM may not transfer cleanly. The [sim-to-real concept page](../concepts/sim-to-real-transfer.md) surveys this for context. JEPA's *latent-space prediction* is theoretically more robust to sim-to-real cosmetic gaps than pixel-prediction models — but this is unproven on educational hardware.
...
- [LeWorldModel — train and run howto](leworldmodel-howto.md) — install, training, eval, gotchas.
...
- [JEPA task capabilities](jepa-task-capabilities.md) — what LeWM can do, generally.
- [Why JEPA research skips the simulator stack](why-jepa-research-skips-the-simulator-stack.md)
- [LLM-agent architecture across stacks](llm-agent-architecture-across-stacks.md) — context for OpenClaw + stretch_ai patterns.
