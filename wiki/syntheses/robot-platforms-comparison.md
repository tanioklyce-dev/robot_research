...
- [LeWM on ROSOrin Pro — feasibility analysis](lewm-on-rosorin-pro-feasibility.md) — uses the research-vs-educational tier distinction extensively.
- [LLM-agent architecture across stacks](llm-agent-architecture-across-stacks.md) — compares stretch_ai (Stretch) vs ROSOrin (cloud/offline) vs OpenClaw (ROSOrin Pro).
- [Sim-heavy vs real-data paths](sim-heavy-vs-real-data-paths.md) — anchors the "what robot underpins each path" question.
