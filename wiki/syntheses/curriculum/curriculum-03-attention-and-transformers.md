...
1. **Get the data.** Use the PushT dataset from [LeWM howto](../world-models/leworldmodel-howto.md) or `diffusion_policy`. Pick ~20 frames at 64×64 resolution.
...
4. **Karpathy's ["Let's build GPT" lecture](../../sources/karpathy-nn-zero-to-hero.md)** (Zero to Hero lecture 7, 1h56m, free) — live-codes a decoder-only transformer from scratch, following Vaswani et al. The video walk-through of what nanoGPT's `model.py` implements; lectures 9 (tokenizer) and 10 (reproduce GPT-2, 4h) continue to a full training run.
