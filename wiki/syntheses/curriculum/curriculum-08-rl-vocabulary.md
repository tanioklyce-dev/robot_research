...
- **Model-Based RL ([MBRL](../../glossary.md#mbrl)).** Learn an explicit dynamics model `f(s, a) → s'` (a [world model](../../concepts/world-models/world-model.md)). Use the model to either *plan* (MPC, [Module 10](curriculum-10-world-models.md)) or *generate synthetic interactions for policy training* (the Dreamer recipe).
