...
Per task, train one predictor head. Five tasks (door, drawer, reorientation, tissue, bag) → five predictors. Single-GPU training, hours per task per the [DINO-WM paper](../sources/dino-wm-paper.md) and [LeWM howto](leworldmodel-howto.md) cost profile.
...
- **Long-horizon assistive tasks.** DINO-WM is short-horizon, image-goal. The [assistive robotics R&D landscape](assistive-robotics-research-landscape.md) flags long-horizon multi-step ADL sequences as one of the seven blocking problems. This experiment does not address it.
...
- [JEPA task capabilities](jepa-task-capabilities.md) — what JEPA models do and don't do.
- [Assistive robotics — R&D landscape](assistive-robotics-research-landscape.md) — broader applicability context.
...
- [Stretch as the de-facto assistive-robotics platform](stretch-as-assistive-platform.md) — why Stretch is the right substrate.
