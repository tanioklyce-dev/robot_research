...
4b. **LeRobot-native robots publish no ROS 2 state — found on hardware, 2026-07-05.** The [XLeRobot](../../entities/xlerobot.md) drives its [SO-ARM101](../../entities/so-arm101.md) arms **directly over the FeeTech USB bus**, so nothing publishes `/joint_states`. The [ros2-mcp-server](../../entities/ros2-mcp-server.md)'s `joint_states()` — and therefore `get_robot_state`, the tool that was meant to be the *first* end-to-end call on a real robot — **has nothing to subscribe to** ([source](../../sources/ros2-mcp-server-github.md#first-real-hardware-validation-2026-07-05-on-the-xlerobot-commits-c89869f--bf2653c)).

   This is the **LeRobot-native ↔ ROS-2 split biting at Layer 1**, and it is structural, not an XLeRobot quirk: the two integration classes named at the top of this page assume a robot either speaks ROS 2 *or* speaks LeRobot, but the agent layer above needs ROS 2 state from *both*. Note the asymmetry with the ROSOrin Pro, which is ROS-2-native and has this for free — so the fleet needs *both* directions of the bridge, not one.

   **Publisher written 2026-07-13 (`8087288`, not yet run on hardware):** [`feetech-joint-states`](../../sources/ros2-mcp-server-github.md#feetech--jointstates-publisher-added-2026-07-13-commit-8087288) — a separate process that reads the FeeTech bus and republishes `sensor_msgs/JointState`. **A stopgap by design, and the reason is worth internalizing:** a serial port has exactly one owner, so this node and LeRobot **cannot both hold the arm bus**. It works when LeRobot isn't driving; it does not solve the general case. The general case is **[Rosetta](../../entities/rosetta.md) owning the bus and serving both** — which makes Rosetta's arm-bus contract a load-bearing fleet dependency, not just a policy-transport nicety. Ladder step 1 (Rosetta contract) is therefore doing more work than it looks.
...
- **A real-time whole-fleet safety layer** — the MCP allowlist covers per-action safety; physical-safety interlocks (e-stop, collision limits) stay in Layer 1.
  - *Follow-up (2026-07-13):* the allowlist turns out to be a **static, name-level [execution rail](../../concepts/safety/ai-guardrails.md)** in the enterprise-guardrail sense — the same abstraction [NeMo Guardrails](../../entities/nemo-guardrails.md) asks for, arrived at independently. **Now partly closed:** [`ros2-mcp-server`](../../entities/ros2-mcp-server.md) gained an **argument-level rail** ([`policy.py`](../../sources/ros2-mcp-server-github.md#execution-rail-added-2026-07-13-commit-b925ddc), commit `b925ddc`) — geofence, keep-out zones, forbidden waypoints, forbidden place targets, enforced in `dispatch()` so mission steps and compiled NL goals hit it too. **Still open:** `pick(knife)` (the rail can't resolve `object_id` → label — needs a detector cache), irreversible *sequences* like `pick(pills)`→`place(trash)` (needs held-object provenance), and — the wide-open one — **[prompt injection through the perception channel](../../concepts/safety/ai-red-teaming.md)**, since a multimodal Layer-2 agent reads text and pixels straight out of the room. Full analysis + the remaining punch list: **[Guardrails for robot agents](../agents/guardrails-for-robot-agents.md)**.
