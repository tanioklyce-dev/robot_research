...
4b. **LeRobot-native robots publish no ROS 2 state — found on hardware, 2026-07-05.** The [XLeRobot](../../entities/xlerobot.md) drives its [SO-ARM101](../../entities/so-arm101.md) arms **directly over the FeeTech USB bus**, so nothing publishes `/joint_states`. The [ros2-mcp-server](../../entities/ros2-mcp-server.md)'s `joint_states()` — and therefore `get_robot_state`, the tool that was meant to be the *first* end-to-end call on a real robot — **has nothing to subscribe to** ([source](../../sources/ros2-mcp-server-github.md#first-real-hardware-validation-2026-07-05-on-the-xlerobot-commits-c89869f--bf2653c)).
   ...
   This is the **LeRobot-native ↔ ROS-2 split biting at Layer 1**, and it is structural, not an XLeRobot quirk: the two integration classes named at the top of this page assume a robot either speaks ROS 2 *or* speaks LeRobot, but the agent layer above needs ROS 2 state from *both*. The fix is a thin **FeeTech → `sensor_msgs/JointState` publisher** (or deferring state to the [Rosetta](../../entities/rosetta.md) contract, which already wraps the arm bus). Note the asymmetry with the ROSOrin Pro, which is ROS-2-native and has this for free — so the fleet needs *both* directions of the bridge, not one.
