...
5. `joint_states` — on LeRobot-native robots this needed a publisher first; **written 2026-07-13** ([`feetech-joint-states`](../../sources/ros2-mcp-server-github.md#feetech--jointstates-publisher-added-2026-07-13-commit-8087288), commit `8087288`, **not yet run on hardware** — start with `--probe`). With it running, `joint_states()` is just a `sensor_msgs/JointState` subscription. Then `speak`, `start_recording` (Rosetta episode_recorder), `estop`.
