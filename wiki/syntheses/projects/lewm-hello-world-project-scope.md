...
Install verified during the [LeWM howto](../world-models/leworldmodel-howto.md) live install on RTX 5070 WSL2. On disk:
...
- Step through `eval.py` once in a debugger or with prints: confirm the model is *not* a policy — it's invoked by the planner as a cost (latent prediction error against a goal embedding). This is the [JEPA](../../concepts/world-models/jepa.md) cost-fn pattern from [JEPA task capabilities](../world-models/jepa-task-capabilities.md) §3.
...
- [LeWorldModel — train and run howto](../world-models/leworldmodel-howto.md) — install / train / eval recipe + the four documented gotchas.
...
- [JEPA task capabilities](../world-models/jepa-task-capabilities.md) — planner-as-cost-fn pattern (§3) and probing/interpretability (§7).
...
- [LeWorldModel howto](../world-models/leworldmodel-howto.md) — recipe-level companion.
