...
Two footnotes from the Rockchip toolchain and its ODMs. The [rknn-toolkit2 operator list](../../sources/rknn-toolkit2-github.md) gives the RKNN column its paper verdict — **ACT plausibly converts, Diffusion Policy is blocked by `GroupNormalization`, SmolVLA-class is out of scope** — the same shape of answer the Hailo column has been waiting for. And an **[RK1828](../../entities/rockchip-rk1828.md) M.2 LLM coprocessor** (5 GB on-package DRAM, claimed 1,024 GB/s, ~5 W, PCIe ×1) is an LLM-shaped occupant for the Pi 5's single lane, the slot the Hailo HAT uses — but every figure for it is one ODM's marketing, whose table also misquotes the Orin NX and the Hailo-8 ([Geniatech](../../sources/geniatech-rk1828-vs-orin-nx-vs-hailo-8.md)). The RK3588's *agent-layer* claim, by contrast, is now **measured rather than assumed**: Rockchip's [RKLLM benchmark](../../sources/rknn-llm-github.md) gives Gemma 4 E2B **11.1 tok/s** and SmolVLM-256M **78 tok/s** on its NPU with an OpenAI-compatible server — better than a Pi 5's CPU, and the only NPU column here with published tokens/s at all (the Hailo-10H still has none). The cost is the same everywhere on this tier: **0.7–3.3 s per image** to encode. The vision column is measured too: Rockchip's [model zoo](../../sources/rknn-model-zoo-github.md) gives YOLOv8n INT8 at **73.5 fps on one RK3588 NPU core** (execution only; three cores available, unmeasured together), Whisper-base at RTF 0.215 — where the Hailo column still has no published fps for any part.
