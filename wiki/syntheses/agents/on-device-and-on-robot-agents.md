...
>
> **Same-model rung, added 2026-09-13**: Rockchip's [RKLLM benchmark](../../sources/rknn-llm-github.md) runs **Gemma 4 E2B on the RK3588 NPU at 11.1 tok/s** (w8a8; 128-token prompt, so TTFT is not comparable to the table) — between the Pi 5 CPU and the Orin Nano GPU. The VLM cost the table hides: **image encoding is 0.7–3.3 s per frame** on that NPU (SmolVLM-256M 0.84 s, Qwen2.5-VL-3B 2.9 s), so a Rockchip-hosted System-2 VLM is a ~1 Hz device before it says a word.
>
> **Speech on the same NPU** ([model zoo](../../sources/rknn-model-zoo-github.md), one RK3588 core, FP16): Whisper-base at **RTF 0.215**, streaming Zipformer ASR at **0.065**, MMS-TTS at **0.069**, YAMNet at 0.004. The agent's ears and voice fit on the accelerator beside the LLM — sharing its LPDDR, not its cores.
