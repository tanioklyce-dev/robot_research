...
The "agentic" framing matters. This survey filters for simulators that support closed-loop control by AI agents — primarily [VLA models](../../concepts/learning/vla-models.md) and [world models](../../concepts/world-models/world-model-simulators.md) — not classical model-based controllers. Six categories worth distinguishing.
...
Both replace authored physics with learned models, but predict different things. See [World-model simulators](../../concepts/world-models/world-model-simulators.md) for the full treatment.
...
Predict next-state *representations* in a learned latent space — no pixels generated. The [Meta FAIR](../../entities/meta-fair.md) / Yann LeCun line. See [Joint-Embedding Predictive Architecture](../../concepts/world-models/jepa.md) for the umbrella concept.
...
- Whether [World-model simulators](../../concepts/world-models/world-model-simulators.md) (Cosmos, Genie Envisioner) can replace physics engines for full closed-loop training, or only complement them, is unsettled.
...
- [Generative-video vs JEPA world models](../world-models/generative-video-vs-jepa-world-models.md) — deeper on §3's two-paradigm split.
- [LLM-agent architecture across stacks](../agents/llm-agent-architecture-across-stacks.md) — deeper on §6's stretch_ai / ROSOrin / OpenClaw convergence.
