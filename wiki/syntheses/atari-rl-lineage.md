...
Hub page tying together the wiki's scattered Atari / DQN material — the [ALE entity](../entities/ale.md), [Sutton & Barto §16.5](../sources/sutton-barto-rl-textbook.md), and [curriculum module 8](curriculum-08-rl-vocabulary.md) — plus the post-DQN algorithmic lineage that the wiki does not otherwise cover. Filed in response to "tell me more about Atari 2600 learning" (2026-05-15 query).
...
DQN is **Q-learning with deep function approximation** plus stabilization tricks ([curriculum-08 §DQN](curriculum-08-rl-vocabulary.md)):
...
See [Module 8](curriculum-08-rl-vocabulary.md) for the policy-gradient vs Q-learning vocabulary.
...
- **PPO** — born on Atari (and MuJoCo locomotion), now the default for sim-to-real legged locomotion ([sim-to-real transfer](../concepts/sim-to-real-transfer.md)) and most large-scale robot RL in Isaac Lab / MuJoCo Playground.
- **MuZero / Dreamer** — the conceptual ancestor of every learned-world-model-plus-planner system in the wiki ([LeWM](../entities/dreamer.md), [TD-MPC](../concepts/world-model.md), [DINO-WM](../entities/dreamer.md)). MuZero solved on Atari what those papers are solving on robotics tasks.
...
- [Curriculum Module 8 — RL vocabulary](curriculum-08-rl-vocabulary.md) — REINFORCE/PPO/DQN/SAC/MFRL/MBRL primer.
- [Optimal control](../concepts/optimal-control.md) — DQN's placement in the deep-RL renaissance timeline.
- [Dreamer](../entities/dreamer.md) — the modern model-based RL line that closes the loop MuZero opened.
- [World model](../concepts/world-model.md) — taxonomy that places MuZero / Dreamer in the broader landscape.
- [Chain of thought](../concepts/chain-of-thought.md) — the LLM-side analogue of "search at decision time" that MCTS provides AlphaGo/MuZero.
