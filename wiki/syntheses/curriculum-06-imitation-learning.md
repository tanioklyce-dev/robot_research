...
> Even if DAgger isn't run literally, "distribution shift" as a concept is *the* reason modern BC pipelines emphasize **demo coverage** — the practical move is to make the demo distribution wide enough that the policy's induced distribution stays in-support. RUM's [data diversity > data quantity](../syntheses/long-term-in-home-robot-deployments.md) finding is essentially this insight under another name.
...
1. **Get the data.** Either:
   ...
   - Use the `stable-worldmodel` PushT shipped with [LeWM howto](leworldmodel-howto.md) and dump `(s, a)` pairs from a few hundred trajectories.
...
1. **[Imitation learning concept page](../concepts/imitation-learning.md)** — short overview and recent canonical references.
