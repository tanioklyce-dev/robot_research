...
2. Stick an [AprilTag](../concepts/apriltags.md) on each bin — [OpenClaw](../entities/openclaw.md)'s chapter-13 demos already include AprilTag-targeted delivery (IDs 0/1).
...
4. Optionally wrap with the LLM agent: *"Sort all the red Legos into bin 0, all the blue Legos into bin 1."* OpenClaw's GPT-orchestrator pattern decomposes this into the existing skill calls ([LLM-agent architecture](../concepts/llm-agent-architecture.md)).
...
- [LLM-agent architecture concept](../concepts/llm-agent-architecture.md) — the OpenClaw orchestration pattern Tier 1 uses.
- [AprilTags concept](../concepts/apriltags.md) — fiducials for bin-pose estimation in Tier 1.
...
- [Stretch as the de-facto assistive-robotics platform](stretch-as-assistive-platform.md) — companion analysis for the next platform tier up.
- [Curriculum Module 6 — Imitation learning and behavior cloning](curriculum-06-imitation-learning.md) and [Module 7 — BC lineage on PushT](curriculum-07-bc-lineage-pusht.md) — pedagogical grounding for the Tier 2 ACT / Diffusion Policy choice.
