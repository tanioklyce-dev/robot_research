...
### Neural geometry
The study of the **shape** of a concept's representation in activation space — as against the assumption that concepts are directions. Established independently by [Engels et al. (MIT, ICLR 2025)](sources/engels2024-not-all-features-one-dimensionally-linear.md), who find **circular** representations of days and months and show that patching only the 2-D circular subspace matches patching the whole layer; extended by [Goodfire](entities/goodfire.md), whose broader claim is that even a scalar concept like position "may live on a curved manifold," with **linear** steering producing teleportation artifacts where manifold-following steering is smooth. **Prevalence is unknown** — nobody can say what fraction of features are non-linear. See [concept page](concepts/safety/neural-geometry.md). *(Safety branch.)*
