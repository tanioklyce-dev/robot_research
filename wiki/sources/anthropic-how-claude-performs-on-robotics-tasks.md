...
- **A third party quotes a number this ingest doesn't contain.** [Introducing Waddle](waddle-labs-introducing-waddle.md) cites this exact source (by its `claude-plays-robotics` URL) for a claim that "adding a movable cursor the model could query for position and depth raised success on a manipulation suite from **6% to 32%**." This ingest recorded end-to-end LIBERO manipulation at **≤5.5%** and found passive **depth heatmaps/crosshairs "roughly neutral"** (a real-world model even *disregarded* the depth channel). A **queryable, movable cursor** is a different intervention from a passive overlay, and Waddle's "manipulation suite" may not be LIBERO-40 — so this is plausibly a distinct result, not a contradiction. **Unresolved:** re-read the primary source for a queryable-cursor manipulation result and the 6%→32% figure. Flagged on the [Waddle source page](waddle-labs-introducing-waddle.md).
