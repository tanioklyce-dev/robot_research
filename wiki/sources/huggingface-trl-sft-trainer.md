...
- **PEFT / [LoRA](../concepts/learning/low-rank-adaptation.md) integration** is first-class — `peft_config=LoraConfig()` is the one-line LoRA setup. Critical for fine-tuning 7B+ VLAs on a single consumer GPU, which is the canonical [SIGRobotics-UIUC](../entities/sigrobotics-uiuc.md) / [XLeRobot](../entities/xlerobot.md) / hackathon workflow.
