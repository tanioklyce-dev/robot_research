...
- **[Locatello et al. 2019](locatello2019-challenging-common-assumptions-disentanglement.md)** — ICML best paper; **ingested 2026-08-30**. The field's major counterpoint to this paper, and it lands harder than "questioning metric robustness": (a) a **theorem** that unsupervised disentanglement is impossible without [inductive biases](../concepts/learning/inductive-bias.md) on *both* model and data, since infinitely many completely-entangled latent spaces share the same marginal distribution; (b) across **>12,000 models**, β-VAE and its five successors *do* enforce what their losses encourage, but **good runs cannot be identified without ground-truth labels**, and **random seeds and hyperparameters matter more than the model choice**; (c) **no validated downstream benefit** — the reduced sample complexity β-VAE's value proposition rests on did not reproduce.
  **Read β-VAE's claims accordingly**: the KL-weighting mechanism is real and the disentanglement metric is this paper's own; what did not survive is the inference that disentanglement is achievable unsupervised or useful downstream.
