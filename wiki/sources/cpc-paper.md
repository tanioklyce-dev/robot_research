...
- **`I(x_{t+k}; c_t) ≥ log N − L_N`** — minimizing InfoNCE maximizes a lower bound on mutual information, and the bound tightens as `N` grows. *This is the origin of the "more negatives is better" folklore*, and of the batch-size sensitivity every contrastive method has inherited since.
...
>
> 1. **The MI lower bound is why negatives scale.** `I ≥ log N − L_N` is a *bound that tightens with N*. Every subsequent complaint about large batches, memory banks and mining strategies ([BYOL](byol-paper.md) §1) traces to this inequality, not to an implementation detail.
...
- [Contrastive learning and InfoNCE](../concepts/learning/contrastive-learning.md) — **this is the origin source**.
...
- **Is the MI bound the right story?** Later work (Poole et al. 2019; Tschannen et al. 2020) argued that InfoNCE's success is not well explained by mutual-information maximization, since the bound is loose and looser bounds sometimes work better. This wiki has **no source on that critique**, and CPC's framing is quoted throughout the field as though settled.
