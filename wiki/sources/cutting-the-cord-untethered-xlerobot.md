...
- **C300 SKU question.** The paper cites its unit (ref [22]) as the **"Anker SOLIX C300 DC Portable Power Station," $159.99**, and says it has **a 12 V/10 A car outlet + 3 USB-C (2×140 W + 1×100 W)**. This conflicts with the wiki's earlier web-sourced claim that the *C300 DC Power Bank lacks a 12 V car port* — see the [Anker comparison](../syntheses/platforms/anker-portable-power-stations.md) contradiction flag. The paper is a built-and-measured primary source, so its port set should be weighted heavily.
