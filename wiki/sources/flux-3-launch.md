...
>
> These two are not straightforwardly compatible. Either (a) tight-tolerance insertion and cable handling are achievable from visual dynamics alone, in which case the force-data argument bounds less than it appears to; or (b) mimic's robots have force or tactile sensing that the blog posts do not mention; or (c) the deployment is narrower than the prose implies. **The posts do not settle it, and this is now the most valuable open question in the wiki's contact-rich thread.** Reading [mimic-video](https://arxiv.org/abs/2512.15692) is the cheapest way to find out.
...
- **Is there force or tactile sensing in mimic's robots?** The single highest-value question here. If tight-tolerance ECU insertion and cable handling are done from visual dynamics alone, the contact-rich literature's force-first consensus is under-described; if there is force sensing, the blog posts omit the component doing the work. **Read [mimic-video](https://arxiv.org/abs/2512.15692).**
