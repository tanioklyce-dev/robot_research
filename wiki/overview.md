...
A starter shortlist — twelve platforms across the spectrum from "3D-print-it-yourself biped" and "tabletop arm under $500" to "research-grade mobile manipulator" and "bipedal humanoid." Each entity page links to a vendor or GitHub product page near the top so you can dig into specs and pricing.

| Platform | Form | Price tier | Why it's on this list |
| --- | --- | --- | --- |
| [Open Duck Mini](entities/open-duck-mini.md) | bipedal walker (no arm) | **~$400–500 in parts**, 3D-printed | The **DIY ancestor of [Microduck](entities/microduck.md)** — [Antoine Pirrone](entities/pollen-robotics.md)'s ~35 cm open-source miniature of Disney's BDX droid (Apache-2.0, STL + BOM + Onshape CAD published, 3.5k stars). Dynamixel **XC330** legs, **Raspberry Pi Zero 2W** running the policies. Its RL ecosystem is the more useful part: `Open_Duck_Playground` ([MuJoCo Playground](entities/mujoco-playground.md) envs) plus a reference-motion generator, with actuator ID via Rhoban **BAM** — the same lineage that reappears in Microduck's [actuator model](concepts/learning/actuator-fidelity-sim2real.md). Google ran **[Gemma 4](entities/gemma4.md) E2B on-device** on two of these at I/O 2026. Choose it over Microduck **only if you want to build and modify the hardware**: the repo says outright it is *"still a work in progress"*, the assembly guide is *"TODO"*, and the alpha has *"too much play at some joints."* Otherwise Microduck is the same money, assembled. |
...
**Looking at humanoids specifically?** The shortlist above stops at the [Unitree G1](entities/unitree-g1.md) because that is the cheapest one you can actually buy. For the rest of the category — the closed-development tier you can't ([Figure 03](entities/figure-03.md), Atlas, Optimus, 1X NEO), the industrial-deployed tier ([Digit](entities/digit.md), [Walden](entities/walden-robotics.md)), the affordable Chinese cluster, and the educational tier below G1 — see **[Humanoid platforms survey](syntheses/platforms/humanoid-platforms-survey.md)**. It sorts ~13 platforms by tier and price, and carries the two arguments worth reading before you assume a humanoid is what you want: **design-for-manufacture** (Figure builds at one robot/hour; nobody else publishes a rate) and **whether the legs are the optional part** — [Walden](entities/walden-robotics.md)'s wheeled base, argued by the man who taught legged locomotion at MIT for twenty years.
...
- **You want the industry mapped:** [The Robot AI industry](syntheses/society/robot-ai-industry-map.md) — the layers, who occupies which, the four structural postures, which automakers own or fund robotics labs, and where the capital comes from. Then [Who benefits from consumer robotics](syntheses/society/consumer-robotics-value-chain.md) for the value-chain reading built up from documented component costs and BOMs.
...
- **You want to know what the wiki thinks is unsolved, and which question it is pointed at:** [Ten open questions, and the one the wiki is pointed at](syntheses/world-models/open-questions-and-research-direction.md) — the open problems ranked, each routed to its primary sources and filed experiments; the focused one is *what information a robot world model must preserve, and how to tell it learned the right representation before it goes on a robot*.
...
Where to go to learn and map this field — the good external resources first, then this wiki's own structured path through it. The scope here is **Robot AI / Physical AI / Embodied AI / robot learning** broadly: the research, its history, and the state of the industry.

### External

- **[awesome-physical-ai](https://github.com/keon/awesome-physical-ai)** (keon; CC0; ~400★, active through 2026-08) — a curated **paper** list organized as a research taxonomy, and the closest thing to a map of the literature: foundations (vision-language backbones, visual representations) → VLA architectures (end-to-end / modular / compact) → action representation (discrete tokenization, discrete-diffusion VLAs, continuous & diffusion policies) → world models (JEPA & latent prediction / generative / embodied) → reasoning & planning → learning paradigms → scaling & generalization → deployment (quantization, real-time control) → safety → lifelong learning → applications (humanoid / manipulation / navigation) → sim-to-real → surveys. Its section structure maps almost one-to-one onto this wiki's `concepts/` folders, which makes it the natural instrument for **coverage-checking** — read a section, ask what this wiki is missing.

  > [!warning] Two different lists share this name
  > This is **keon/awesome-physical-ai**. The wiki separately ingested **[natnew/awesome-physical-ai](sources/awesome-physical-ai-github.md)** on 2026-07-08 — a different repository with the same name, different curator, different license, and a different shape (a full-stack directory of simulators, datasets, courses, companies and hardware rather than a paper taxonomy). They are complementary, not duplicates. Do not merge them or treat a claim about one as a claim about the other.

- **[Robotic Manipulation: Perception, Planning, and Control](https://manipulation.csail.mit.edu/index.html)** ([Russ Tedrake](entities/russ-tedrake.md), MIT CSAIL) — the standard graduate course notes for manipulation, maintained as **interactive HTML** with runnable [Drake](entities/drake.md)-based notebooks (the author treats the HTML as canonical; the downloadable PDF is autogenerated and lags). Kinematics, geometric perception, motion planning, force control, and a model-based treatment of the same problems the learning literature attacks end-to-end.

  This is the **counterweight to almost everything else in this wiki**. The curriculum below is learning-first; these notes are the model-based tradition, written by the person who runs both — Tedrake led [TRI](entities/tri.md)'s [Large Behavior Models](concepts/learning/large-behavior-models.md) program while TRI funds Drake, which is this wiki's clearest institutional evidence that the two programs are complementary rather than successive. Read alongside, not instead.
...
>
> Read this section as **one serious bet on how world models should work**, worth understanding deeply — not as the wiki's answer. Where that leaves the research direction is spelled out in [Ten open questions, and the one the wiki is pointed at](syntheses/world-models/open-questions-and-research-direction.md). The broader question is [generative-video vs JEPA](syntheses/world-models/generative-video-vs-jepa-world-models.md), and it is open.
