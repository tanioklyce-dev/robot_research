...
- [Sutton & Barto — Reinforcement Learning: An Introduction (2nd ed., in-progress draft)](sources/sutton-barto-rl-textbook.md) — UMass / UAlberta / DeepMind; A Bradford / MIT Press. The canonical RL textbook. Defines the field's four-subelement decomposition (policy / reward / value function / model) and the unifying narrative MC ↔ TD ↔ DP via Bellman bootstrapping. Ch 1, 3, 4, 6, 8 are the wiki-relevant foundations; Ch 11 is the policy-gradient bridge to PPO / actor-critic. The primary-source anchor for [Module 8 — RL vocabulary](syntheses/curriculum-08-rl-vocabulary.md), every MBRL paper ([DreamerV3](sources/dreamer-v3-paper.md), [TD-MPC2](sources/td-mpc2-paper.md)), every learned-WM thread, and the RLHF/DPO/GRPO line underneath every VLA. The "RL = approximate optimal control under uncertainty" bridge to [Sussmann & Willems 1997](sources/sussmann-willems-1997-300-years-optimal-control.md). (2014–2015 draft; final 2018)
