...
- **Independently observed outside sports (2026-07-22).** A first-party pinball tracker ([pinball_tracker](../sources/pinball-tracker-repo.md)) trained on one machine scores **0.914 F1** on a second machine and **0.555 F1** on a third. Cross-*venue* degradation is real and sizeable (~0.36 F1 here), and the two held-out venues differ substantially with no reliable way to predict in advance which kind a new venue will be — so the weakness generalizes past "sport" to **any change of venue with bespoke visual content**. Magnitude is closer to V1's cross-*court* result (recall 97.3 → 75.7) than its cross-*sport* one.

  > [!warning] An earlier version of this bullet claimed a near-exact reproduction of V1's tennis→badminton **35.2 F1 / 22.9% recall**, citing 0.232 F1 for the third machine. **That was a data bug** — a 5-frame label misalignment — and the claim is withdrawn. The corrected figure is 0.555. See [fast-ball-tracking §9](../syntheses/projects/fast-ball-tracking-for-robots.md) for the correction and the method note.
