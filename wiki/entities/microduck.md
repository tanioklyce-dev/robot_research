...
- **Lineage.** Microduck descends from **[Open Duck Mini](open-duck-mini.md)**, **Antoine Pirrone**'s ~35 cm 3D-printable BDX-droid miniature. Now established from primaries rather than press: Pirrone is a credited Microduck author and a Pollen R&D engineer, and **Pollen's own `policies/README.md` states the shipped ONNX files were *"copied from `apirrone/microduck_runtime`"*** ([runtime repo](../sources/microduck-runtime-repo.md)). No Pollen *marketing* material names Open Duck Mini.
...
- [airockchip/rknn_model_zoo](../sources/rknn-model-zoo-github.md) — the RK3566 column that corroborates the onboard yolo11n measurement
