...
**A full-graph TensorRT path exists.** The [Seeed DLI course](../sources/seeed-nvidia-dli-rebot-sim-to-real-course.md) builds **seven engines** for GR00T 1.7 — `vit`, `llm_bf16`, `vl_self_attention`, `state_encoder`, `action_encoder`, `dit_bf16`, `action_decoder` — at bf16/batch-1 on both AGX Thor and AGX Orin under JetPack 7.2, i.e. the whole model compiled rather than the official recipe's DiT-head-only compile. It uses a **third-party fork pinned to a commit** (`jjjadand/Isaac-GR00T-Orin-JP72` @ `dcf5f6b`), not NVIDIA's repo. **No latency figure is printed in the course text** (benchmark output survives only as screenshots), so this does not yet displace the 10.9 Hz number — but it establishes the path is reproducible, and if it beats DiT-only the [Jetson inference table](../syntheses/platforms/gr00t-inference-on-jetson.md) needs a new row. Engines are strictly target-specific: never copy them between Orin and Thor.

**Backbone flexibility.** N1.7's default VLM is `nvidia/Cosmos-Reason2-2B` (Hub license acceptance required). That same pinned fork validates **`Qwen/Qwen3-VL-2B-Instruct`** as a drop-in via `GR00T_BACKBONE_PATH` — described as *"a validated alternative for this checkpoint and repository revision, not the official GR00T 1.7 default."* Since Cosmos-Reason2-2B is itself Qwen3-VL-derived, this is less a swap than a retreat to the base model.

**Action horizon is a ceiling, not a requirement.** The wiki records N1.7 expanding the action horizon 16 → 40. The reBot embodiment config sets `delta_indices=list(range(0, 16))` — **horizon 16** — and trains fine. It also splits action representations per modality: **`single_arm` RELATIVE** ("delta from current state, better generalization"), **`gripper` ABSOLUTE** ("binary open/close works better absolute"), both `NON_EEF` joint-space. That matches the reference SO-101 recipe's "relative actions excluding gripper," which makes it two independent instances of the same convention rather than a one-off.
...
- [A Sim-to-Real VLA Pipeline with Seeed reBot Arm and NVIDIA Isaac](../sources/seeed-nvidia-dli-rebot-sim-to-real-course.md) — seven-engine full-graph TensorRT on Thor/Orin; Qwen3-VL-2B alternative backbone; a worked `NEW_EMBODIMENT` config
