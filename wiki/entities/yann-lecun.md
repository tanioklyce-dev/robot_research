...
- **Domestic robots: a precondition, never a date.** His only ingested statement on household robots in his own words is the [MIT Technology Review interview (2026-01-22)](../sources/mit-tech-review-lecun-ami-labs-interview.md): *"nobody — absolutely nobody — knows how to make those robots smart enough to be useful. Take my word for it,"* because they need *"an enormous amount of tele-operation training data for every single task"* and do not generalize when the environment changes. The bar he sets is *"a domestic robot that is as agile as a house cat,"* and the condition is *"that's not going to happen until we have good world models and planning."* **He gives no year for household deployment anywhere the wiki can find** — the timelines he does commit to are for the *architecture* — 3–5 years for a new paradigm — and for AMI Labs' industrial plan below, which excludes robots. The fullest version of the position is the [AI House Davos conversation](../sources/ai-house-davos-2026-lecun-embodied-ai.md) the same week as the interview: *"none of those companies, absolutely none of them, has any idea how to make those robots smart enough to be useful — it's a big secret of the robotics industry,"* the dishwasher-loading 10-year-old as the zero-shot standard, and *"that's the challenge for the next few years: getting a system to really understand the real world"* — a clock on the world model, again not on the robot. The closest he has come to a robot timeframe is [Davos, January 2025](../sources/wef-davos-2025-debating-technology.md), under an audience slide headed *"consumer robotics"*: *"**maybe** the coming decade will be the decade of robotics, **because maybe** we'll have AI systems that are sufficiently smart to understand how the real world works."* Two hedges, and the second is the condition; secondary coverage kept the first and dropped the dependency.
- **The 3–5-year clock and the 10¹⁴-bytes argument both originate at Davos 2025** ([primary](../sources/wef-davos-2025-debating-technology.md)): *"the shelf life of the current paradigm… is fairly short, probably three to five years. I think within five years nobody in their right mind would use them anymore, at least not as the central component"* — a claim that falls due **January 2030**. And the arithmetic he repeats everywhere since: a frontier LLM's 20–30 T tokens ≈ 10¹⁴ bytes ≈ what a four-year-old's optic nerves carry in 16,000 waking hours, hence *"never going to get to human-level AI by just training on text."*
- **"LLMs are intrinsically unsafe, because they're not controllable"** — and *"not particularly dangerous because they're not particularly smart either"* (same session). Guardrails trained in can be trained out by leaving the fine-tuned domain; his promised alternative is objective-driven systems whose guardrails are *"hardwired"* into the reasoning and cannot be prompt-jailbroken. On alignment faking: *"they don't have any intentions… they don't have any values."* On regulation: governments *"brainwashed… into believing in the existential risk story."* See [AI guardrails](../concepts/safety/ai-guardrails.md) for how this sits against the wiki's finding that every rail above the physical interlock is behavioral. See [JEPA for a household mobile manipulator](../syntheses/world-models/jepa-for-household-mobile-manipulator.md) for how this sits against the wiki's own estimates.
...
- [Debating Technology — WEF Davos 2025 town hall (video, 2025-01-23)](../sources/wef-davos-2025-debating-technology.md) — **primary for "decade of robotics"** (conditional, two hedges), the 3–5-year LLM shelf life, the 10¹⁴-bytes argument, and "LLMs are intrinsically unsafe."
...
- [LeCun & Xing debate (2026)](../sources/lecun-xing-jepa-glp-debate-2026.md) — the wiki's only primary of LeCun arguing JEPA *against a named opponent*. Opens with *"world models must not be generative"*; the entropy / `PV = nRT` argument; *"whatever information you cannot predict is not going to be useful. I guarantee that"*; MAE *"basically failed"*; Jupiter in six numbers. Concedes text (*"no objection to using text in a JEPA"*). Names SIGReg and LeWorldModel as the current instruments.
- [The Birth of SSL — A Supervised Theory](../sources/birth-of-ssl-supervised-theory-paper.md) — with Balestriero (2024 workshop note).
