...
- **2025 Spot + Gemini Robotics hackathon** ([source](../sources/bostondynamics-spot-gemini-robotics.md)). Two Spot-team engineers (Issac Ross and Nikhil Devraj) wired [Gemini Robotics-ER 1.5](gemini-robotics.md) into Spot via a thin tool-call layer (`GoTo`, `TakePicture`, object ID, `Pickup`, `PutDown`) over the Spot SDK. Demonstrated living-room cleanup from handwritten natural-language instructions. Architecture is a textbook [LLM-agent](../concepts/llm-agent-architecture.md) pattern.
