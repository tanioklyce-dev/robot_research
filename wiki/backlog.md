...
- [ ] **The runtime-monitoring cluster PACS cites is entirely uningested** — three 2025 papers on knowing when a generative policy is going wrong: Agia, Sinha, [Pavone](entities/marco-pavone.md) & Bohg (CoRL 2025, failure modes of generative policies); **Xu et al. (RSS 2025, TRI** — Ambrus, Nishimura, Itkina — detecting failures *without failure data*); Römer et al. (NeurIPS 2025, failure prediction at runtime). Directly adjacent to the wiki's [policy-evaluation](concepts/robotics/robot-policy-evaluation.md) thread and to the empty [execution rail](syntheses/agents/guardrails-for-robot-agents.md).
