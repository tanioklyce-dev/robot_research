...
## [2026-07-27] Project Fetch ingest — follow-ups
- [ ] **Unitree Robotics company entity** — the wiki now has [G1](entities/unitree-g1.md), [H1](entities/unitree-h1.md), and [Go2](entities/unitree-go2.md) but **no parent-company page**, and every one of them links to bare-text "Unitree Robotics". Deliberately not filed during this ingest: no ingested source carries citable company facts (founding, Wang Xingxing, Hangzhou, funding, IPO). File when a source that actually grounds it lands.
- [ ] **Ingest *How Claude Performs on Robotics Tasks*** (anthropic.com/research/claude-plays-robotics) — the apparent **autonomy-side** companion to [Project Fetch](sources/anthropic-project-fetch-robot-dog.md)'s uplift-side result, same Frontier Red Team. Would let the wiki state both halves of the uplift→autonomy claim from primary sources instead of one.
- [ ] **Confirm the Project Fetch robot.** The post never names it; [Unitree Go2](entities/unitree-go2.md) is filed on secondary coverage with a warning callout. A primary confirmation (Anthropic photo caption, a participant's post, the companion page) would retire the caveat.
- [ ] **Claude 4 System Card p. 114** — the prior "Claude trains a quadruped locomotion policy in sim, not yet autonomously capable" evaluation is cited by Project Fetch but un-ingested. Would be the wiki's first system-card ingest and the actual baseline the uplift result is measured against.
- [ ] **Quadruped tier is the thinnest-sourced platform group.** Neither [Spot](entities/spot.md) (vendor blog) nor [Go2](entities/unitree-go2.md) (a policy article that doesn't name it) has a primary technical source, and **no ingested paper uses a quadruped** — while quadruped locomotion RL is a major real subfield. Flagged on the [robot platforms comparison](syntheses/platforms/robot-platforms-comparison.md).
- [ ] **Uplift measurement is a one-source concept.** [AI uplift studies](concepts/safety/ai-uplift.md) rests entirely on Project Fetch. The biological-risk uplift studies it descends from are referenced but un-ingested; METR-style time-horizon / developer-productivity RCTs would be the obvious second and third anchors.
