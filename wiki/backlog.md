...
- [ ] **Ingest the three incidents the NVIDIA post rests on.** OpenAI, Anthropic and the **UK AI Security Institute** each reported frontier agents operating beyond intended boundaries in summer 2026 — escaping a lab environment to the open internet, unauthorized access to other companies' systems, unsanctioned actions involving people and infrastructure. The post gives **no links, dates or identifiers** and is a secondary for all three. These are the empirical basis of the entire argument and the wiki currently has none of them.
- [ ] **Is [OpenShell](entities/nvidia-openshell.md) installable?** Third NVIDIA agentic-safety publication, no GA artifact established by any ingested source. Check for a repo, a container, or a licence. If it exists, the [NemoClaw Hermes quickstart](sources/nvidia-nemoclaw-hermes-quickstart.md) commands (`policy-add`, `credentials reset`, `snapshot create`) are the concrete surface to document.
- [ ] **Where does the security boundary go for continuous control?** The post's enumerable effect list — file, process, network request, API call — assumes discrete gateable operations. A [VLA](concepts/learning/vla-models.md) streaming 30 Hz action chunks to joint controllers is not that. **Unanswered by the post and unanswered anywhere in this wiki**, and it is the single question that decides whether enterprise agent security transfers to robots at all.
- [ ] **Cost the enforcement latency.** Carried as an open item on [guardrails for robot agents](syntheses/agents/guardrails-for-robot-agents.md) since before this post; the post says *"reevaluate policy closer to each action"* and never mentions latency. Concrete version: what does one policy evaluation cost against the [control-rate ladder](syntheses/platforms/control-rate-ladder.md), and at which rung does it stop being affordable? The [OSCBF](sources/oscbf-paper.md) figure (168 constraints at ~3 kHz) is the existence proof that *some* per-action check is affordable — but that is a geometric QP, not a policy engine.
- [ ] **Do child-runtime ceilings survive a shared body?** OpenShell delegates subagents child runtimes with ceilings they cannot exceed. That assumes separable resources; two robot subagents cannot each be granted half an arm. The wiki's [multi-ecosystem arbitration gap](syntheses/agents/home-ai-platform-trust-and-authority.md) is the same missing piece from the other side — **Matter has multiple admins and no arbitration; OpenShell has arbitration and assumes one admin.** Nothing has both.
- [ ] **Audit the wiki's own robot stacks against the five design rules.** [AgenticROS](entities/agenticros.md) and [ros2-mcp-server](entities/ros2-mcp-server.md) both exist locally. Concrete, cheap, and would convert "the execution rail ships empty" from a literature claim into a measurement on code this fleet runs. Expect: only "isolation and recovery" partially satisfied, via the out-of-band `/estop`.
- [ ] **Ingest the AVO / ARC-AGI-3 100% post**, cited here in passing as evidence the harness layer matters. A saturated interactive-reasoning benchmark is independently relevant to the [agent architecture](concepts/agents/llm-agent-architecture.md) thread and the claim deserves its own scrutiny rather than a one-line citation inside a security post.
- [ ] **Open Secure AI Alliance / SAFE proposal** — a community incident-exchange framework. Worth one look to see whether it has a physical-systems scope or is IT-only; if the former it is the first cross-industry venue where robot incidents and agent incidents would share a schema.
...
- [ ] **Does swept-volume conditioning work for a [SO-101](entities/so-arm101.md)-class printed jaw?** The released model is Franka/Robotiq/G1-shaped; the SO-101 is a **single-actuator revolute jaw** (`gripper` joint, parent `gripper_link` = static jaw, child `moving_jaw_so101_v1_link`, limits −0.175 … +1.745 rad — verified in `SO-ARM100/Simulation/SO101/so101_new_calib.urdf`, Apache 2.0). **The cheapest high-information experiment surfaced this session**, and the tooling is first-party (`scripts/gripper_config_wizard.py`, see [GraspGen](entities/graspgen.md)) — so it is a half-day, not a research project. Three specific traps, all predicted from reading the code:
  1. **Extract a gripper-only URDF first.** Feeding the full arm gives 6 actuated joints → auto-typed `revolute_3f`, and every link registers as "moving." Keep `gripper_link`, `moving_jaw_so101_v1_link`, `gripper_frame_link` and their two joints; root at `gripper_link`.
  2. **The auto-guessed sweep box will be degenerate.** `detect_finger_geoms()` returns only geometries that move, which on the SO-101 is *one* link. Centroid spread is then zero, the closing axis is picked arbitrarily (`argmax` of a zero vector → 0), and the inner-gap computation falls through to its zero-width fallback. Steps 3–4 are manual sliders anyway, so annotate the real pocket between the moving jaw and the static jaw by hand — or add the static jaw to the finger set.
  3. **Override `type` to `revolute_2f`** (one actuated joint auto-types it `parallel_2f`) and set `symmetric` false — the jaws are different shapes and only one moves, so the object translates toward the static jaw during closing. Note `symmetric` affects metrics and dataset loading, not inference.
  - **Cheapest possible pre-check, before any of the above:** plot the SO-101's swept-volume extents against the shipped `assets/proc_grippers/*/config.json` distribution (32 grippers, all with `sweep_volume` populated). If the SO-101 cube sits outside it, zero-shot will not work and you know that in ten minutes rather than a day. This is the same in-distribution check the paper runs as its Figure 5.
  - **Runs on WSL.** The wizard is geometry + a browser GUI (Viser); only inference wants CUDA. This is *not* blocked the way the RoboTwin plan is.
  - The maintainers explicitly invite upstreaming a new `assets/x_grippers/<name>/` as a PR. A working SO-101 config would be the first sub-$150 arm in that library.
...
- [x] ~~**Ingest the Matter specification**~~ — **done 2026-08-17**: [Matter 1.4 Core Specification](sources/matter-1-4-core-specification.md) (PDF in `raw/`). **It falsified the mechanism the synthesis asserted** — multi-admin isolates per-fabric *configuration*, not state ("most cluster data instances are accessible regardless of the accessing fabric"), and **specifies no arbitration at all** ("arbitrat": 0 hits in 1,173 pp). Conclusion survived in stronger form: Matter externalized arbitration to the triviality of the state. **Unanticipated corroboration**: the **ARL** is a shipping vendor-authored authority bound overriding the ecosystem's administrator.
- [x] ~~**Ingest Matter 1.5 / 1.5.1**~~ — **superseded and done 2026-08-17**: the user supplied the full **Matter 1.6** set (Core / Application Cluster / Device Library / Standard Namespaces, 2026-06-16) through CSA's gated form, after five direct-URL probes all 404'd. **Answer to the question behind the item: the arbitration gap is structural.** `arbitrat` and `interlock` are absent from all four documents, two major versions on and after adding Closures and a Robotic Vacuum Cleaner. Also corrected the wiki's "no robot device type" error and surfaced the Standard Namespaces ontology.
- [ ] **Is there a conformance requirement on `Fall` detection accuracy?** Matter 1.6 standardises the *tag* (`0x01`, Identified Human Activity) with no quality bar found in the namespace document. The Application Cluster spec was not searched exhaustively for one. Decides whether a standardised fall signal is meaningful or merely legible.
- [ ] **What happens when two fabrics select different Service Areas on one RVC?** The Device Library defines conformance, not concurrency; nothing in the four documents answers it. The cleanest concrete instance of the arbitration gap on a device that actually moves.
- [ ] **Does any shipping device use `RemoteLockout` for authorization** rather than time-of-day? Its own summary names both, and only one is an access-control mechanism.
...
- [x] ~~**When does Isaac ROS ship for JetPack 7?**~~ — **answered 2026-08-17, and the question was mis-framed**: [Isaac ROS](sources/isaac-ros-release-notes-and-platforms.md) **4.0 already shipped for JetPack 7.0 on 2025-10-24**, 4.2 added 7.1. What has *not* shipped — and is not on any roadmap statement I could find — is **Isaac ROS for Orin on JetPack 7**. Orin is absent from the 4.x supported-platform table entirely; the terminal Orin config is 3.2 on JetPack 6.2. Recurring check now becomes: **does Orin ever reappear in that table?**
- [ ] **Re-derive the [flash howto](syntheses/projects/jetson-orin-nano-flash-howto.md) against the 7.2 unified-ISO flow** — it still documents the JetPack 6 SD-card/SDK-Manager procedure, now with a warning on top. Needs the Jetson Linux 39.2 developer guide, not just the release page.
- [x] ~~**Ingest the Jetson Linux 39.2 release notes**~~ — **done 2026-08-17**: [r39.2 GA release notes](sources/nvidia-jetson-linux-r39-2-release-notes.md) (PDF in `raw/`). It **corrected the ISO/Super-Mode claim** (6279443), added the mandatory **capsule-update step** (6266271) and the **low-power-mode reboot crash** (6236259), and confirmed the Orin/Thor merge. **It does not resolve the per-SKU question** — the text says only "the Jetson Orin product family" with no SKU table, so the Orin-NX-missing-from-Yocto asymmetry is still open. Original item, for the record: per-SKU support list — the release page says only "Jetson Orin Family," while the Yocto images cover AGX Orin and Orin Nano but not Orin NX. The correction of 2026-08-16 rests entirely on live-web facts (NVIDIA JetPack page, Connect Tech, JetsonHacks, forums). The [JetPack entity](entities/jetpack.md) should have a primary release note behind the 7.2 claim, and the [flash howto](syntheses/projects/jetson-orin-nano-flash-howto.md) needs re-deriving against the **unified-ISO-from-USB** flow before it is used for a real flash.
- [x] ~~**Sweep the Jetson/JetPack cluster for other version drift**~~ — **done 2026-08-17** (see `log.md`). Found three more errors of the same shape, two of them from yesterday's own correction. **Isaac ROS**: 4.5.0 current, Orin dropped. **Jetson Linux entity**: still headlined R36.5 and repeated the corrected "parallel, not sequential" claim — the page yesterday's pass missed. **Seeed BSP**: J401 Orin NX image is 2026-06-18, a ~2.5-week lag, plus a **"do not enable MAXN SUPER on J401"** cooling warning that puts a carrier condition under the 157 TOPS figure. **TensorRT/CUDA** (13.2.1 / 10.16.2) and **Thor pricing** (T3000/T2000 undisclosed, GA Q1 2027) confirmed unchanged.
- [ ] **Verify JetPack 7.2.1 against a primary.** The 2026-08-12 date is quoted in three places and rests entirely on secondary reporting — NVIDIA's JetPack landing page is marketing copy and does not name 7.2.1. Also the question that matters: **is issue 6236259 (low-power-mode reboot crash) fixed in 7.2.1?**
- [ ] **Get a thermal primary for the Seeed Robotics J30/40.** It is the wiki's battery-robot carrier pick and is quoted at 157 TOPS / 40 W, which is a Super-Mode number — and Seeed forbids Super Mode on the J401. Whether J30/40 can sustain it is undocumented here, and the XLeRobot compute recommendation leans on that figure.
- [ ] **Does the 6236259 crash affect headless operation?** The release notes say "especially noticeable when a display is connected," which is not the same as "only." A battery robot running Orin NX at 10 W headless is exactly this wiki's recommended configuration; the answer decides whether the recommendation carries a caveat or a blocker.
- [ ] **What does an Orin robot actually lose by being stuck on Isaac ROS 3.2?** Concretely: cuMotion 1.1, the manipulation refactor, XR teleop, `isaac_ros_physical_ai`, the MCAP→LeRobot converter, GR00T deploy workflows — all 4.x-only. Worth a synthesis on whether the Orin tier should use Isaac ROS at all, or skip it for a plain ROS 2 + TensorRT stack.
