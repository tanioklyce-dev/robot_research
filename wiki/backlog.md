...
- [ ] **Ingest Matter 1.5 / 1.5.1** — *(2026-08-17: five direct-URL patterns probed against the csa-iot uploads path, all 404. The 1.4 trick does not generalize; this needs the gated download form, i.e. a human with a browser.)* — the wiki is one major version behind. 1.5 adds **cameras** (first Matter class whose data sensitivity approaches a home robot's) and **closures** (first that physically moves). **Whether 1.5 added arbitration or interlocks for closures decides whether the gap identified here is structural or incidental.** 1.5's PDF is not directly linkable; the download form 403s unauthenticated, so this needs the gated route.
