...
- [~] **ROS 2 ↔ MCP server** — **design + skeleton built 2026-07-04**: [design doc](syntheses/projects/ros2-mcp-server-design.md) in the wiki + the `ros2-mcp-server` sibling repo (MIT skeleton: config-driven tools, result envelope, rclpy stubs to wire). Remaining: wire the `ros_bridge` ROS 2 calls, add SSE transport, **push to GitHub → ingest as a source page** (`sources/ros2-mcp-server-github.md`) + entity, per the "document code repos, don't contain them" pattern.
