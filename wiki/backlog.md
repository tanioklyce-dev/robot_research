...
- [ ] **Nobody has run a learned policy through a CBF filter.** [OSCBF](sources/oscbf-paper.md) has the guarantee and teleop-only experiments; the [TRI stack](sources/diffusion-policy-paper.md) has the policies and no guarantee. Their own future work is exactly this composition. **What a safety filter costs a diffusion policy in task success is unmeasured anywhere in this corpus** — and per the [success-rate audit](syntheses/platforms/vla-success-rate-audit.md), measuring it properly needs ~1,000 rollouts.
