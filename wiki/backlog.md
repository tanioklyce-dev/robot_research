...
- [ ] **Put the 83 Hz figure on one axis with the wiki's edge-latency numbers.** The eval's 83 Hz-needed / 0.2–0.4 Hz-achieved gap is for an *LLM in the loop*; the [Jetson ladder](syntheses/platforms/jetson-module-ladder-power-performance.md) and [XLeRobot onboard compute](syntheses/platforms/jetson-onboard-compute-xlerobot.md) hold 1.4–27.8 Hz for *on-edge VLAs*. Different systems at different abstraction levels, never compared. A short synthesis would be genuinely new work and the pieces are all in the wiki already.
