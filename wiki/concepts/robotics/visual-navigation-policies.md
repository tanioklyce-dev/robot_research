...
| **[LeLaN](../../sources/lelan-paper.md)** | CoRL 2024 | ~50M ResNet-FiLM+Transformer / 130 h **action-free** video (82.5 h YouTube, 32 countries) | **language** (object) | labels *and* actions synthesized (SAM → CogVLM → GPT-3.5; NoMaD + depth for actions); 54 ms on Orin | 0.89 vs 0.67 best baseline over 1,050 trials; YouTube data never saturates |
| **[CAST](../../sources/cast-paper.md)** | arXiv 2025-08 | PaliGemma 3B / GNM + 86K counterfactual chunks | language (complex) | **counterfactual labels** to defeat posterior collapse; audited labels ~60–70% correct | +27 pts over the same VLA on hindsight labels; 2× on manipulation with distractors |
...
Adjacent RAIL work the papers cite: RECON and ViKiNG (latent-goal exploration; kilometer-scale with geographic hints), FastRLAP / SELFI / lifelong-improvement (RL on navigation models), SACSoN (social navigation data), ExAug (the forward-model objective MBRA reuses), LeLaN and CAST are now ingested (rows above). CityWalker (Feng, CVPR 2025) is the parallel web-video line; NaVILA the legged-robot VLA that OmniVLA found prompt-incompatible.
...
- **Data grew 60 h → 100 h → 800 h → 9,500 h**, and almost all of the growth is **relabeled or synthesized actions** (LeLaN's 130 h via NoMaD + depth, FrodoBots via MBRA, BDD-V via a custom relabeler) with **synthesized language** on top (LeLaN's 20M VLM prompts; CAST's Gemini labels, the only ones audited: **60–70% correct**). The human-labeled core never exceeded ~100 h. The wiki's [crowdsourcing](../learning/crowdsourced-robot-training-data.md) page records the consequence: crowdsourced *actions* were worth zero.
