...
- **[DimOS](../../entities/dimos.md)** ([Dimensional Inc.](../../entities/dimensional-inc.md), commercial open-core; [repo](../../sources/dimos-github.md)) — the **largest instance of the pattern in this wiki** (3,874★, 12+ contributors, Apache 2.0), and the one that changes how the pattern is best described. Three moves distinguish it: (1) **the agent is an ordinary module** subscribing to the same camera / LiDAR / odometry / spatial-memory streams as everything else, not a separate supervisory process; (2) **the tool surface is discovered, not written** — decorate any module method with `@skill` and the agent enumerates it over RPC at startup, docstring becoming the tool description, so the manifest cannot drift from the code; (3) **[MCP](#mcp--model-context-protocol) is the interface**, with **[LangGraph](../../entities/langgraph.md)** above it and `McpServer` open to any external client. Skills span `relative_move`, `observe`, `navigate_with_text`, `tag_location`, `follow_person`, `speak`, `map_query`. Default planner is cloud `gpt-5.6-luna`; [Ollama](../../entities/ollama.md) is an alternate blueprint. No success rates published.

- **[Flexion Reflect](../../entities/flexion.md)** ([v0](../../sources/flexion-reflect-v0.md), [v1.0](../../sources/flexion-reflect-v1.md)) — the pattern's **most capable instance**, on a humanoid, and the first to **measure** it. A **custom, RL-fine-tuned VLM** as mission control over RL-trained whole-body skills **and a VLA**; semantic-map queries in natural language; missions respecified by prompt and interruptible mid-run. Their design statement is the best one-line summary of the whole pattern: *"language drives intent and physics enforces feasibility."* They also identify why off-the-shelf VLMs fail long-horizon work — *"they emit the next logically plausible tool call too quickly"* instead of **verifying the previous one completed** — and fix it with **RL fine-tuning**: 16-step mission end-to-end completion **38% (SFT) → 90% (SFT+RL)**, n unstated.
...
> [!note] Where the tool schema comes from is the axis that has actually moved
> The stacks above split three ways on how the LLM learns what it can do. **Hand-written prompt manifest** (stretch_ai, ROSOrin, OpenClaw) — simple, and drifts from the code. **Agent-authored skills** ([Waddle](../../entities/waddle-labs.md)) — the library grows from experience and is shared across agents, which is the most ambitious claim and the least evidenced. **Introspected from the running system** ([DimOS](../../entities/dimos.md)) — `@skill` decorators discovered by RPC, so the manifest is a projection of the deployed code rather than a parallel artifact. The third is the one a large codebase converges on for maintenance reasons, and it is the design most robotics teams should copy.
