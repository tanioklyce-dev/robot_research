...
| **[VideoPhy](../../sources/videophy-paper.md)** (2024) | Physical commonsense in T2V, **human-judged** (3 annotators, majority vote, 36.5K labels) on 688 captions across solid-solid / solid-fluid / fluid-fluid; best model **39.6%** joint, solid-solid worst; motion vs physics r = −0.8; GPT-4V / Gemini near chance as judges (ROC-AUC 53 / 58), a fine-tuned 7B judge 73 — ranking survives, level does not | **ingested** |
| **[VideoPhy-2](../../sources/videophy-2-paper.md)** (2025) | Action-centric: 197 actions → 3,940 prompts, **102K human annotations** incl. per-video followed/violated labels on LLM-written, law-tagged rules; joint SA∧PC; best (Wan2.1-14B) **32.6%**, **21.9% on the hard subset**; mass and momentum conservation violated ~40%; auto-evaluator only r = 0.37 with human physics ratings | **ingested** |
| **[PhyGenBench](../../sources/phygenbench-paper.md)** (2024) | 160 prompts each engineered around one of **27 textbook laws** (mechanics / optics / thermal / material); **PhyGenEval** hands the judge the law as yes/no questions → Spearman **0.81** with humans (VideoPhy's metric: 0.04 on the same videos); best Gen-3 **0.51**; scaling CogVideoX 2B→5B moves mechanics 0.38→0.39; a VBench-boosting enhancer adds **0.00** | **ingested** |
| **[WorldModelBench](../../sources/worldmodelbench-paper.md)** (2025) | Judges video models *as world models*: 350 text+first-frame prompts, 7 domains incl. robotics and driving; 10-point rubric (instruction following / commonsense / five physics laws); 67K crowd labels; Kling v1.5 **8.82** yet only **61%** of tasks fully completed; a fine-tuned 2B VILA judge matches leaderboard totals within 4.1% but **over-scores 13 of 14 models**; VBench correlates **0.28** with physics adherence | **ingested** |
| **Cosmos-Eval** / **PhyWorldBench** | Physical commonsense in generated video, model-judged | not ingested |
...
| **[WorldScore](../../sources/worldscore-paper.md)** (2025) | 3D, 4D, I2V and T2V models on one leaderboard via **camera-controlled next-scene generation**: 3,000 specs (2,000 static / 1,000 dynamic), ten automated metrics in controllability / quality / dynamics, validated by a 400-participant 2AFC study for 5 of the 10. **3D models beat video models on static** (WonderWorld 72.7 vs CogVideoX-I2V 62.2); video models cannot follow a camera (40 vs 85–94); open CogVideoX beats Gen-3 and Hailuo. **No physics metric at all** — 'dynamics' is optical-flow presence; 3D models are zeroed on dynamics, so the dynamic leaderboard is partly an artifact; four metrics are cohort-z-scored, so later snapshots are not comparable | **ingested** |
...
The progression VBench → VideoPhy/PhyGenBench → WorldScore/WorldModelBench → WorldArena runs from *how it looks* toward *what it is good for* — with two corrections now that the primaries are read. **WorldScore sits on a *control* axis (camera, object, content), not a physics or utility one** — a renderer benchmark; **WorldModelBench is an output-rubric benchmark, not a utility one**: crowd and VLM scoring of clips, nothing about planning or policy use; it belongs beside VideoPhy. And **PhyGenBench measures correctness of a *declared* outcome** — it can only score a video whose physics was written down in advance, which is not the case a robot world model faces. WorldArena and WorldRoamBench sit at the far end and have primary sources here.

> [!note] The human-versus-judge finding is two years older than this page said (corrected 2026-09-11)
> The callout below credits Physion-Eval (2026) with discovering that model judges cannot see physics. [VideoPhy](../../sources/videophy-paper.md) reported it in **June 2024**: GPT-4V and Gemini-1.5-Pro score **53 and 58 ROC-AUC** (chance = 50) against human physics labels, and a 7B judge fine-tuned on 12K of those labels reaches 73 — *ranking preserved, level not*. [VideoPhy-2](../../sources/videophy-2-paper.md) repeats it in 2025 (Gemini-2.0 r = 0.11, fine-tuned 7B r = 0.37 on physics). [PhyGenBench](../../sources/phygenbench-paper.md) reaches ρ = 0.81 **only by handing the judge the law** — its own ablation of an unguided naturalness judge is τ ≈ 0.19 for GPT-4o and near zero for a video model. [WorldModelBench](../../sources/worldmodelbench-paper.md)'s judge inflates 13 of 14 models. Physion-Eval's contribution is the human *reasoning traces* and the 0.1 s localization, not the discovery. Also: every one of these four benchmarks is **human-labeled at the gold layer**; the earlier "scored automatically or by a model judge" description was wrong for them.
>
> A second tension to hold: per-law violation rates disagree across instruments by an order of magnitude — WorldModelBench ≤12% per law (5-item crowd checklist, ~1.7 votes, 4 s clips) vs Physion-Eval's 83–94% of clips with ≥1 violation (expert traces). Instrument, not model, difference; unresolved.
...
Note a small closed loop in the policy record: **WorldScore was co-authored by two of the HAI brief's own authors** (Fei-Fei Li, Jiajun Wu), and its first author Haoyi Duan appears on WorldArena 2.0. The brief presents the benchmark landscape as external evidence without noting the overlap. The primary tightens it: **the top model on WorldScore-Static, WonderWorld, and the fourth-ranked WonderJourney are by the benchmark's own first authors and senior author**, undisclosed in the paper, and WonderJourney is one of the five models in its human study ([WorldScore](../../sources/worldscore-paper.md)). The backlog's question — does WorldScore's framing support or undercut the brief? — resolves to *supports*: its stated scope is entertainment, education and simulation; 'embodied AI' appears once and is never tested.
...
- [VideoPhy](../../sources/videophy-paper.md) — the founding generative-video physics benchmark; human-judged; first 'judges near chance' result (2024).
- [VideoPhy-2](../../sources/videophy-2-paper.md) — action-centric, per-law human rule annotations; the hard-subset ceiling of 21.9%.
- [PhyGenBench](../../sources/phygenbench-paper.md) — law-scaffolded evaluation; the VEnhancer null.
- [WorldModelBench](../../sources/worldmodelbench-paper.md) — video models judged as world models; judge inflation; VBench–physics correlation 0.28.
- [WorldScore](../../sources/worldscore-paper.md) — camera-controlled next-scene generation across 3D/4D/video models; no physics metric; authorship loop with the top-ranked model.
